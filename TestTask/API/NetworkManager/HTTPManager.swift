...
class HTTPManager {
    ...
    //MARK: - GET
    class func get(url: UrlRequest, paramInUrl: String? = nil, params: [String: Any]?, successHandler: SuccessHandler? = nil, failureHandler: FailureHandler? = nil) {
        
        var requestUrl = url.fullUrl
        
        if let paramInUrl {
            requestUrl += paramInUrl
        }
    ...
    private class func handleSuccessResponse(url: String, data: Data, successHandler: SuccessHandler? = nil, failureHandler: FailureHandler? = nil) {
        do {
            ...
            failureHandler?(ApiError(propertyName: LocalizeStrings.networkError, displayMessage: LocalizeStrings.networkErrorMsg, errorCode: ErrorCode.errorParsing))
        }
    }
