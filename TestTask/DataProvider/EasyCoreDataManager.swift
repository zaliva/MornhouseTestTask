...
final class EasyCoreDataManager {
    ...
    private init() { 
        ...
        guard let modelURL = bundle.url(forResource: "FactDataModel", withExtension: "momd") else {
            fatalError("Failed to find model URL")
        }
        guard let model = NSManagedObjectModel(contentsOf: modelURL) else {
            fatalError("Failed to load model")
        }
        persistentContainer = NSPersistentContainer(name: "FactDataModel", managedObjectModel: model)
    }
    ...
    func loadPersistentStores(completion: @escaping ((LoadPersistentStatus) -> Void)) {
